    ...
    def redraw(self):
        if self._display_error:
            ...
            for cid, choice in self._choices.items():
                ...
                print(f'{sel} ({choice.id}):\t{choice.text}')

            if self._ok is not None:
                sel = self.selector_symbol(self._ok)
                print(f'{sel} ({self._ok.id}):\t{self._ok.text}')
            if self._cancel is not None:
                sel = self.selector_symbol(self._cancel)
                print(f'{sel} ({self._cancel.id}):\t{self._cancel.text}')

...
logger = get_logger("gwent.hal.matrix")
...
class MatrixDisplay:
    ...
    def __init__(self, i2c=None, address=0x74, mux=None, mux_channel=None):
        ...
        try:
            ...
            logger.info(f"Initialized matrix display with address 0x{address:02x}")
        except Exception as e:
            logger.error(f"Error initializing matrix display: {e}")
            raise
    ...
    def clear(self):
        ...
        logger.debug("Matrix display cleared")
    ...
    def draw_border(self, brightness=255):
        ...
        logger.debug(f"Drew border with brightness {brightness}")
    ...
    def draw_text(self, text, brightness=50, scroll=True, scroll_delay=0.1):
        ...
        if scroll:
            ...
            logger.debug(f"Scrolled text: '{text}' with brightness {brightness}")
        else:
            ...
            logger.debug(f"Displayed text: '{text}' with brightness {brightness}")
    ...
    def fade(self, fade_in=500, fade_out=500, pause=1000):
        ...
        logger.debug(f"Fading display with fade_in={fade_in}ms, fade_out={fade_out}ms, pause={pause}ms")
...
class MultiMatrixDisplay:
    ...
    def __init__(self, mux_address=0x70, matrix_address=0x74, channels=None):
        ...
        try:
            ...
            logger.info(f"Initialized multi-matrix display with {len(channels)} displays")
        except Exception as e:
            logger.error(f"Error initializing multi-matrix display: {e}")
            raise
    ...
    def clear_all(self):
        ...
        logger.debug("Cleared all matrix displays")
    ...
    def draw_border_all(self, brightness=255):
        ...
        logger.debug(f"Drew borders on all displays with brightness {brightness}")